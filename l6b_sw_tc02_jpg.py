...
class ETL:
    ...
    def __init__(self, collection_jpg, fs_jpg):
        ...
        # 連結資料庫
        self.collection, self.fs = connect_MongoDB(client='mongodb://ivan:ivan@10.88.26.183:27017', db_name="AT", collection="L6B_SW_TC02_charge2d")
    ...
    def etl(self):
        ...
        if df.empty:
            ...
            df = df.drop_duplicates(['lm_time','eqp_id','op_id','recipe_id','lot_id','sheet_id','chip_id','step','ins_cnt','charge_type'])
            df_all_sheet = df.drop_duplicates(['eqp_id','op_id','recipe_id','lot_id','sheet_id','step','ins_cnt','charge_type'])
    ...
    def plot_sheet(self, df, X, Y, H, W,
        ...
        for color in ["Reds","Greens","Blues"]:
            ...
            plt.savefig('l6b_sw_tc02_temp.jpg')
            image = Image.open("l6b_sw_tc02_temp.jpg")
            image_bytes = io.BytesIO()
            image.save(image_bytes, format="JPEG")
            image_bytes.seek(0)

            sheet_2d_object_id_lst.append(self.fs_jpg.put(image_bytes, filename="l6b_sw_tc02_temp.jpg"))
...
def job():
    ...
    logging.basicConfig(filename=f'log/l6b_sw_tc02_jpg_{datetime.today().strftime("%Y_%m_%d_%H_%M")}.log', filemode='w+', format='%(name)s - %(levelname)s - %(message)s', level=logging.INFO)
    ...
    db = client["AT_jpg"]
    # 選擇 collection
    collection_jpg = db["L6B_SW_TC02_JPG"]
    # 選擇 gridfs
    fs_jpg = gridfs.GridFS(db, collection="L6B_SW_TC02_JPG")    
    ...
    logging.info("ETL L6B SW TC02 JPG")
    print("ETL L6B SW TC02 JPG")  
    etl_obj.etl()
